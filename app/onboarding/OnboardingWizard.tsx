...
export default function OnboardingWizard() {
  ...
  function canAdvance() {
    ...
    if (step === 2) return true // title + experience optional
    return true
  }
    ...
    >
            ...
            {stepLabels.map((label, i) => {
              ...
              return (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '.4rem' }}>
                    <div
                      style={{
                        ...
                        background: active ? 'var(--green)' : done ? 'var(--green)' : 'var(--white)',
                        display: 'flex',
                        alignItems: 'center',
                  ...
                  {i < 2 && (
                    <div style={{ width: 24, height: 1, background: 'var(--rule)', margin: '0 .3rem' }} />
                  )}
                </div>
              )
            })}
        ...
        <div style={{ flex: 1, padding: '3rem', maxWidth: 520 }}>
          ...
          {step === 2 && (
            <div className="animate-fade-up">
              ...
              <p style={{ fontSize: '.88rem', color: 'var(--muted)', marginBottom: '2.5rem', fontWeight: 300, lineHeight: 1.7 }}>
                A short bio that helps people understand your background and what you're looking for.
              </p>
          ...
          {step === 3 && (
            <div className="animate-fade-up">
              ...
              <p style={{ fontSize: '.88rem', color: 'var(--muted)', marginBottom: '2.5rem', fontWeight: 300, lineHeight: 1.7 }}>
                Select the industries and company stages you've worked in. This helps recruiters and hiring managers find the right fit.
              </p>
                    ...
                    {INDUSTRIES.map((ind) => (
                      <button
                        ...
                        style={{
                          ...
                          background: form.industries.includes(ind) ? 'var(--green)' : 'var(--white)',
                          color: form.industries.includes(ind) ? '#fff' : 'var(--muted)',
                          fontSize: '.78rem',
                          fontWeight: form.industries.includes(ind) ? 600 : 400,
                          cursor: 'pointer',
                          fontFamily: 'var(--sans)',
                          transition: 'all .15s',
                        }}
                    ...
                    {STAGES.map((s) => (
                      <button
                        ...
                        style={{
                          ...
                          background: form.stages.includes(s) ? 'var(--green)' : 'var(--white)',
                          color: form.stages.includes(s) ? '#fff' : 'var(--muted)',
                          fontSize: '.78rem',
                          fontWeight: form.stages.includes(s) ? 600 : 400,
                          cursor: 'pointer',
                          fontFamily: 'var(--sans)',
                          transition: 'all .15s',
                        }}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2.5rem' }}>
            ...
            {step < 3 ? (
              <button
                ...
                style={{ opacity: canAdvance() ? 1 : 0.4, fontSize: '.88rem', padding: '.8rem 1.8rem' }}
              >
                Continue →
              </button>
            ) : (
              <button
                ...
                className="btn-primary"
                style={{ fontSize: '.88rem', padding: '.8rem 1.8rem' }}
              >
      ...
      >
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
          <p style={{ fontSize: '.65rem', fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--muted)' }}>
            Live preview
          </p>
        </div>
        ...
        <div style={{ padding: '1.5rem', flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
            ...
            <div>
              <p style={{ fontFamily: 'var(--serif)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--ink)' }}>
                {form.name || 'Your name'}
              </p>
          ...
          {form.location && (
            <p style={{ fontSize: '.75rem', color: 'var(--muted)', marginBottom: '.75rem' }}>
              📍 {form.location} {form.remote_preference && `· ${form.remote_preference}`}
            </p>
          )}

          {form.bio && (
            <p style={{ fontSize: '.82rem', fontWeight: 300, lineHeight: 1.7, color: 'var(--ink2)', marginBottom: '1rem' }}>
              {form.bio}
            </p>
          )}
          ...
          >
            <p style={{ fontSize: '.62rem', fontWeight: 600, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--green2)', marginBottom: '.6rem' }}>
              ● Former manager
            </p>
            <p style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: '.82rem', lineHeight: 1.65, color: 'var(--ink2)', marginBottom: '.75rem' }}>
              "Your first vouch will appear here once a colleague verifies their email."
            </p>
